...
int move_check(int id,int dir)//returns 0 for blocked, 1 for open, 2 for screen change
{
	...
	if(map->map[y][x].space!=-1 || map->map[y][x].type==2 || map->map[y][x].type==6 || (player->type==1 && (map->map[y][x].type==4 || map->map[y][x].type==3)))
		return 0;
	return 1;
}
...
void npc_move(int id,int dir)
{
	cPlayer *player = playerStruct(id);
	if(player->type!=1)
		return;
	cMap *map=mapStruct(player->map);
	int res = move_check(id,dir);
	if(res==2)
	{
		if(dir==DIR_UP)
		{
...
int npc_follow(int id,int target)//chases target and returns 1, if target lost returns 0
{
	cPlayer *player = playerStruct(id);
	if(player->type!=1)
		return 0;
...
void setHP(int xPlayerID, int xHP)
{
	...
	if (xHP < pVictim->hp)	// If HP are decreasing
		...
		if (pVictim->type == PLAYER_TYPE_PC && pAttacker->type == PLAYER_TYPE_PC && (pVictim->serenity > 0 || pAttacker->serenity > 0))	// If this is player vs player and either player has serenity, nullify damage
			return;

...
struct cChatBalloon
{
	...
	void draw()
	{
		...
		if(id==me)
		{
			...
			else if(player[me].dir==2){x+=smove;cx+=smove;}
			else if(player[me].dir==3){x-=smove;cx-=smove;}
		}
		else
		{
			int path=getPath(id);
			...
			else if(player[id].dir==2){x+=path;cx+=path;}
			else if(player[id].dir==3){x-=path;cx-=path;}
		}
...
{
	me=0;player[0].access=10;chatting=0;strcpy(world[MYGUY.p.map].name,"test");

...
/* Some macros for accessing the (now multi-purpose)
    TCPsocket->sflag:
*/
#define SDL_NET_TCP_FLAG_SERVER 0x1
#define SDL_NET_TCP_FLAG_NONBLOCKING 0x2
#define SDL_NET_TCP_IS_SERVER(s) (s->sflag & 0x1)
#define SDL_NET_TCP_IS_NONBLOCKING(s) (s->sflag & 0x2)
...
TCPsocket SDLNet_TCP_Open(IPaddress *ip)
{
	...
	// ##########  Binding locally
		...
		sock->sflag = SDL_NET_TCP_FLAG_SERVER;
	}
	sock->ready = 0;
...
TCPsocket SDLNet_TCP_Accept(TCPsocket server)
{
	...
	/* Only server sockets can accept */
	if ( ! SDL_NET_TCP_IS_SERVER(server) ) {
		SDLNet_SetError("Only server sockets can accept()");
		return(NULL);
	}
	server->ready = 0;
...
int SDLNet_TCP_Send(TCPsocket sock, void *datap, int len)
{
	...
	/*do {
		...
		if ( len > 0 ) {*/
	if ( SDL_NET_TCP_IS_NONBLOCKING(sock) )
	{
		do 
		{
			len = send(sock->channel, (const char *) data,left, 0);
		...
		if ( (len < 0) && (errno == EAGAIN || errno == 10035) ) //eagain != 10035??
		{
			sent = 0; /* Not an error here */
		} 
		else 
		{
			sent += len;
...
int SDLNet_TCP_Recv(TCPsocket sock, void *data, int maxlen)
{
	...
	if ( len )
	{
		if(len==SOCKET_ERROR )
		{
			...
			if( SDL_NET_TCP_IS_NONBLOCKING(sock) && errno==WSAEWOULDBLOCK) 
				len = 0; /* Not an error here */
			else
				len = -1;
		}
	}
...
/* Set the socket to blocking or non-blocking, if supported by the
    networking system.
    Returns 0 on success, and non-zero on failure.
*/

int SDLNet_TCP_SetNonBlocking(TCPsocket sock, int nonblocking) 
{
	if ( SDL_NET_TCP_IS_SERVER(sock) ) 
	{
		SDLNet_SetError("Cannot change blocking mode on server sockets");
		return -1;
	}
	...
	SDLNet_SetError("Non-blocking sockets not supported on this platform");
	return -1;  /* skip rest */
	...
	if (nonblocking)
	{
		sock->sflag |= SDL_NET_TCP_FLAG_NONBLOCKING;
	} 
	else 
	{
		sock->sflag &= ! SDL_NET_TCP_FLAG_NONBLOCKING;
...
/* Get the non-blocking status of a socket, as in SetNonBlocking.
    Returns 1 if the socket is non-blocking, and 0 if the socket
    is a regular, blocking socket.
*/
int SDLNet_TCP_GetNonBlocking(TCPsocket sock)
{
	return SDL_NET_TCP_IS_NONBLOCKING(sock);
}

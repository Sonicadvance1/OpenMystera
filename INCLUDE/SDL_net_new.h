...
/* Set the socket to blocking or non-blocking, if supported by the
    networking system. Non-blocking sockets always return instantly
    from Send and Recv calls.
    The "non-blocking" name was chosen over the "blocking" variant to
    emphasize that blocking is the default mode, and the setting must
    be actively changed to get a non-blocking socket.
    Returns 0 on success, and non-zero on failure.
 */
extern DECLSPEC int SDLCALL SDLNet_TCP_SetNonBlocking(TCPsocket sock, int nonblocking);
 ...
 */
extern DECLSPEC int SDLCALL SDLNet_TCP_GetNonBlocking(TCPsocket sock);
